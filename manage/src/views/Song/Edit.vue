    ...
    <el-card v-loading="loading">
      ...
      >
        ...
        <el-form-item label="音乐文件" prop="file">
          ...
          >
            <el-button type="primary" :icon="Upload">替换音乐文件（可选）</el-button>
            <template #tip>
...
import { getSongDetail, updateSong } from '@/api/song'
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        const res = await updateSong(route.params.id, formData)
        if (res.code === 0) {
